...
public:
    state(enclosing_fsm_type& fsm) : state::state_type{}, fsm_{&fsm} {}
...
class inner_state_machine
    : public detail::state_machine_base<T, none, inner_state_machine<T, FSM>> {
public:
    using enclosing_fsm_type = FSM;
...
public:
    inner_state_machine(enclosing_fsm_type& fsm) : base_machine_type{this}, fsm_{&fsm} {}
...
private:
    ...
    template <typename Event>
    actions::event_process_result
    process_event_impl(Event&&, detail::process_type<actions::event_process_result::refuse> const&)
    {
        static_assert(detail::event_process_selector<Event, typename state_machine::handled_events,
            ...
            while (!deferred.empty()) {
                ...
                for (auto event = deferred.begin(); event != deferred.end();) {
                    if (handled_.count(event->second)) {
                        ...
                        deferred_events_.splice(deferred_events_.end(), deferred, event, next);
...
private:
    ...
    {
        static_assert(
            detail::event_process_selector<Event, typename priority_state_machine::handled_events,
...
template <typename T, typename FSM>
auto
root_machine(inner_state_machine<T, FSM>& fsm) -> decltype(root_machine(fsm.enclosing_fsm()))
{
...
template <typename T, typename FSM>
auto
root_machine(inner_state_machine<T, FSM> const& fsm) -> decltype(root_machine(fsm.enclosing_fsm()))
{
    return root_machine(fsm.enclosing_fsm());
}
...
template <typename T, typename FSM>
auto
root_machine(state<T, FSM>& fsm) -> decltype(root_machine(fsm.enclosing_fsm()))
{
    return root_machine(fsm.enclosing_fsm());
}
...
template <typename T, typename FSM>
auto
root_machine(state<T, FSM> const& fsm) -> decltype(root_machine(fsm.enclosing_fsm()))
{
    return root_machine(fsm.enclosing_fsm());
}

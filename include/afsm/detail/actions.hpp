...
{
    return res == event_process_result::process || res == event_process_result::process_in_state;
}

namespace detail {

template <typename Action, typename Event, typename FSM, typename SourceState, typename TargetState>
...
struct handles_event : std::conditional<std::is_same<typename Transition::event_type, Event>::value,
                                        std::true_type, std::false_type>::type {};

template <typename Guard, typename FSM, typename State, typename Event>
struct guard_wants_event
    : std::integral_constant<
          bool, psst::meta::is_callable<Guard, FSM const&, State const&, Event const&>::value> {};
...
struct guard_check
    : std::conditional<
          ...
          typename std::conditional<psst::meta::is_callable<Guard, FSM const&, State const&>::value,
                                    guard_event_check<false, FSM, State, Event, Guard>,
                                    guard_check<FSM, State, Guard, none>    // TODO Replace with
                                                                            // concept assert
                                    >::type>::type {};
...
struct action_invocation_impl {
    ...
    {
        static_assert(action_long_signature<Action, Event, FSM, SourceState, TargetState>::value,
...
struct action_invocation_impl<Action, Event, FSM, SourceState, TargetState, false> {
    ...
    {
        static_assert(action_short_signature<Action, Event, FSM>::value,
                      "Action is not callable for this transition");
        Action{}(std::forward<Event>(event), fsm);
    }
};

template <typename Action, typename FSM, typename SourceState, typename TargetState>
struct action_invocation {
    ...
    operator()(Event&& event, FSM& fsm, SourceState& source, TargetState& target) const
    {
        using invocation_type = action_invocation_impl<
            Action, Event, FSM, SourceState, TargetState,
            action_long_signature<Action, Event, FSM, SourceState, TargetState>::value>;
...
template <typename Action, typename Guard, typename FSM, typename SourceState, typename TargetState>
...
struct nth_inner_invocation {
    ...
    using invocation_type = guarded_action_invocation<action_type, guard_type, FSM, State, State>;
...
struct in_state_action_invocation {
    ...
    using handler_type = typename std::conditional<
        ...
        detail::unconditional_in_state_invocation<fsm_type, state_type,
                                                  typename event_handlers::template type<0>>,
        detail::conditional_in_state_invocation<fsm_type, state_type, event_handlers>>::type;
...
handle_in_state_event(Event&& event, FSM& fsm, State& state)
{
    ...
    return in_state_action_invocation<FSM, State, decayed_event>{}(std::forward<Event>(event), fsm,
                                                                   state);
}
...
public:
    ...
    template <typename Event>
    using invocation_table
        = std::array<std::function<event_process_result(states_tuple&, Event&&)>, size>;

public:
    explicit inner_dispatch_table() {}

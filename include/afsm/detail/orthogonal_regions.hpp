...
public:
    regions_table(fsm_type& fsm) : fsm_{&fsm}, regions_{regions_constructor::construct(fsm)} {}
    regions_table(fsm_type& fsm, regions_table const& rhs)
        : fsm_{&fsm}, regions_{regions_constructor::copy_construct(fsm, rhs.regions_)}
    {}
    regions_table(fsm_type& fsm, regions_table&& rhs)
        : fsm_{&fsm}, regions_{regions_constructor::move_construct(fsm, ::std::move(rhs.regions_))}
    {}

    regions_table(regions_table const&) = delete;
    regions_table(regions_table&& rhs) : fsm_{rhs.fsm_}, regions_{::std::move(rhs.regions_)} {}
    regions_table&
    operator=(regions_table const&)
        = delete;
    regions_table&
    operator=(regions_table&&)
        = delete;
...
public:
    regions_stack(fsm_type& fsm) : fsm_{&fsm}, state_stack_{stack_constructor_type::construct(fsm)}
    {}
    ...
    regions_stack&
    operator=(regions_stack const&)
        = delete;
    regions_stack&
    operator=(regions_stack&&)
        = delete;
...
struct region_container {
    ...
    region_container(FSM* fsm) : regions_{*fsm} {}
    region_container(FSM* fsm, region_container const& rhs) : regions_{*fsm, rhs.regions_} {}
    region_container(FSM* fsm, region_container&& rhs) : regions_{*fsm, ::std::move(rhs.regions_)}
    {}
...
template <typename FSM, typename FSM_DEF, typename Size>
struct region_container
    : detail::region_container<FSM, FSM_DEF, Size, def::has_pushdown_stack<FSM_DEF>::value> {
    using base_type
        = detail::region_container<FSM, FSM_DEF, Size, def::has_pushdown_stack<FSM_DEF>::value>;
    using region_tuple = typename base_type::region_tuple;

    region_container(FSM* fsm) : base_type{fsm} {}
    region_container(FSM* fsm, region_container const& rhs) : base_type{fsm, rhs} {}
    region_container(FSM* fsm, region_container&& rhs) : base_type{fsm, ::std::move(rhs)} {}

...
TEST(VendingMSM, OnOff)
{
    ...
    EXPECT_TRUE(vm.is_flag_active<vending_def::off>()) << "Vending machine is off";
    EXPECT_FALSE(vm.is_flag_active<vending_def::on>()) << "Vending machine is not on";

    EXPECT_TRUE(vm.process_event(events::power_on{})) << "Vending machine turns on correctly";

    EXPECT_FALSE(vm.is_flag_active<vending_def::off>()) << "Vending machine is not off";
    EXPECT_TRUE(vm.is_flag_active<vending_def::on>()) << "Vending machine is on";
    ...
    EXPECT_TRUE(vm.process_event(events::power_off{})) << "Vending machine turns off correctly";
...
TEST(VendingMSM, Maintenance)
{
    ...
    EXPECT_TRUE(vm.is_flag_active<vending_def::off>()) << "Vending machine stays off";
    ...
    EXPECT_TRUE(vm.is_flag_active<vending_def::on>()) << "Vending machine stays on";
    EXPECT_TRUE(vm.is_flag_active<vending_def::on::out_of_service>())
    ...
    // Exit maintenance
    EXPECT_TRUE(vm.process_event(events::end_maintenance{})) << "Vending machine exits maintenance";
    EXPECT_TRUE(vm.is_flag_active<vending_def::on>()) << "Vending machine stays on";
    EXPECT_TRUE(vm.is_flag_active<vending_def::on::out_of_service>())
    ...
    // Load some goods
    EXPECT_TRUE(vm.process_event(events::load_goods{0, 10})) << "Vending machine consumes goods";
    EXPECT_TRUE(vm.is_flag_active<vending_def::on::maintenance::idle>())
        << "Vending machine transits back to idle maintenance mode";
    EXPECT_FALSE(vm.is_empty());
    EXPECT_EQ(10, vm.count());
    EXPECT_TRUE(vm.process_event(events::load_goods{1, 100})) << "Vending machine consumes goods";
    EXPECT_TRUE(vm.is_flag_active<vending_def::on::maintenance::idle>())
        << "Vending machine transits back to idle maintenance mode";
    EXPECT_FALSE(vm.is_empty());
    ...
    // Set prices
    EXPECT_TRUE(vm.process_event(events::set_price{0, 10.0})) << "Set price for item 0";
    EXPECT_TRUE(vm.process_event(events::set_price{1, 5.0})) << "Set price for item 1";
    EXPECT_FALSE(vm.is_empty());
...
TEST(VendingMSM, BuyItem)
{
    ...
    EXPECT_FALSE(vm.is_flag_active<vending_def::off>()) << "Vending machine is not off";
    EXPECT_TRUE(vm.is_flag_active<vending_def::on>()) << "Vending machine is on";
    EXPECT_TRUE(vm.is_flag_active<vending_def::on::serving>()) << "Vending machine is serving";
    EXPECT_TRUE(vm.is_flag_active<vending_def::on::serving::idle>())
        << "Vending machine is serving";

    EXPECT_TRUE(vm.process_event(events::money{3})) << "Vending machine accepts money";
    ...
    EXPECT_TRUE(vm.process_event(events::money{5})) << "Vending machine accepts money";
